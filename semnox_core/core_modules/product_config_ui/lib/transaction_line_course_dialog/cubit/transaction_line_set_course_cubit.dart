...
class TransactionLineSetCourseCubit extends Cubit<TransactionLineSetCourseState>{
  TransactionLineSetCourseCubit(): super(TransactionLineSetCourseState()); // here inside TransactionLineSetCourseState class,  all the fields are already initialized
  ...
  Future<void> updateTransactionLineCourse(
      {String? lineRemarks,
        int?approverID,
      String? lineCourseName,
      List<int>? transactionLineIds}) async {
    try {
      ...
      transactionResponse =
          transactionResponse = await transactionDataBl.updateTransactionLineCourse(
          approverId:approverID,
          courseName: lineCourseName??'',
          lineRemarks: lineRemarks??'',
          transactionLineIds: transactionLineIds??[-1]);
      emit(state.copyWith(
          transactionResponse: transactionResponse!,
          isError: false,
          isLoading: false,
