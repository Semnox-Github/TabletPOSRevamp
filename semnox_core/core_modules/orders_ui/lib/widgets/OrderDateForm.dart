...
class OrderDateForm extends StatefulWidget{
  TextEditingController? controller ;
  String title;

  OrderDateForm({super.key, required this.controller, required this.title});

  @override
  State<OrderDateForm> createState() => _OrderDateFormState();
}

class _OrderDateFormState extends State<OrderDateForm> {
      ...
      children: [
        ...
        TextFormField(
          ...
          decoration: InputDecoration(
              ...
              hintText: "Enter",
              hintStyle: theme.subtitle3?.copyWith(fontWeight: FontWeight.w500, fontSize: SizeConfig.getFontSize(16)),
              suffixIcon: Padding(
          ...
          onTap: () async {
            ...
            if(birthDate != '') {
              ...
              DateFormat dateFormat = DateFormat(_dateFormat ?? 'dd-MMM-yyyy h:mm a');
              try{
            ...
            if(pickedTime != null && pickedDate != null){
              ...
              formattedDate = DateFormat(_dateFormat ?? 'dd-MMM-yyyy h:mm a').format(DateFormat("dd-MM-yyyy hh:mm:ss a").parse(formattedDate));
              widget.controller?.text = formattedDate;
            }else if(pickedDate != null){
              String formattedDate = DateFormat('dd-MM-yyyy hh:mm:ss a').format(pickedDate);
              formattedDate = DateFormat(_dateFormat ?? 'dd-MMM-yyyy h:mm a').format(DateTime(pickedDate.year, pickedDate.month, pickedDate.day, 4, 0, 0));
