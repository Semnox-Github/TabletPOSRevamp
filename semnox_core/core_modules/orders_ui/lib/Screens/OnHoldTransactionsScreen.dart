...
import '../widgets/TransactionCustomerInputForm.dart';

class OnHoldTransactions extends StatefulWidget {
  const OnHoldTransactions({Key? key}) : super(key: key);

  @override
  State<OnHoldTransactions> createState() => _OnHoldTransactionsState();
}

class _OnHoldTransactionsState extends State<OnHoldTransactions> {
  ...
  void initState() {
    ...
    // String formattedDate1 = DateFormat('dd-MMM-yyyy hh:mm:ss a').format(DateTime.now().subtract(Duration(days: 1)));
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      setNotification.value = NotificationMessage(message: MessagesProvider.get("View &1 transactions ",['on hold']),color:theme.footerBG4);
    });
  }
  ...
  Widget build(BuildContext context) {
                        ...
                        child: Column(
                          children: [
                            ...
                            Visibility(
                                ...
                                child: TransactionFilter(trxNoController: _trxNoController,trxIdController: _trxIdController,
                                  orderNoController: _orderNoController,otpController: _otpController,referenceController: _referenceController,
                                  customerNameController: _customerNameController,fromDateController: _fromDateController,toDateController: _toDateController,
                                  ...
                                  onSearch: (){
                                    if(!_isDateGreater(_toDateController.text,_fromDateController.text)){
                                      ...
                                      _search();
                                      WidgetsBinding.instance.addPostFrameCallback((_) {
                                        setNotification.value = NotificationMessage(message: MessagesProvider.get("View &1 transactions ",['Onhold']),color:theme.footerBG4);
                                      });
                                    }
                                  },)),
                                  ...
                                  children: [
                                    ...
                                    Container(
                                        ...
                                        child:TransactionRHS(
                                          dateFormat: _dateFormat,
                                          selectedIndex: _selectedIndex,currentPage: _currentPage,pageSize: _pageSize,
                            ...
                            TablePagination(currentPage: _currentPage, totalPage: _totalPage, pageController: _pageController,
    ...
    ).withLoader(theme,isLoading: _isLoading, message: MessagesProvider.get("Loading Onhold Transactions"));
  ...
  _search() async{
    ...
    try{
      Log.printMethodStart('_search()', 'Orders Screen','on hold');
      var response = await orderSearchBL.getTransaction(
        ...
        status: (_status?.statusDescription == null || _status?.statusDescription == "All") ? "BOOKED,CONFIRMED,ORDERED,"  : _status?.statusDescription,
      ...
      Log.printMethodEnd('_search()', 'Orders Screen','on hold');
      if(response.exception == null) {
        Log.printMethodReturn('_search() - done', 'Orders Screen','on hold');
