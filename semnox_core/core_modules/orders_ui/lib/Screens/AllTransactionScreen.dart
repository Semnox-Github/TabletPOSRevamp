...
TransactionStatusContainerDTO? status;
...
class _AllTransactionScreenState extends State<AllTransactionScreen> {
  ...
  void initState() {
   ...
   // String formattedDate1 = DateFormat('dd-MMM-yyyy hh:mm:ss a').format(DateTime.now().subtract(Duration(days: 1)));
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      setNotification.value = NotificationMessage(message: MessagesProvider.get("View &1 transactions ",['All']),color:theme.footerBG4);
    });
  }
  ...
  Widget build(BuildContext context) {
                                ...
                                child: Row(
                                  children: [
                                    ...
                                    Container(
                                        ...
                                        child:TransactionRHS(selectedIndex: _selectedIndex,currentPage: _currentPage,pageSize: _pageSize,
                                          dateFormat: _dateFormat,
    ...
    ).withLoader(theme,isLoading: _isLoading, message:MessagesProvider.get("Loading All Transactions"));
  }
  ...
  _search() async{
    ...
    try{
      ...
      var response = await orderSearchBL.getTransaction(
          ...
          pageSize: _pageSize,
       //  trxNumber: _trxNoController.text == "" ? 0 : _trxNoController.text.contains(RegExp(r'[a-z]')) ? 0 :int.parse(_trxNoController.text),
          ...
          posMachine: posMachine?.posMachineId ?? _posMachineId,
          status: (status?.statusDescription == null || status?.statusDescription == "All") ? "BOOKED,CONFIRMED,MERGED,ORDERED,PAID,PENDING_CLOSE,REOPENED,STARTED,CLOSED,CANCELLED,ABANDONED"  : status?.statusDescription,
      ...
      if(response.exception == null) {
        Log.printMethodReturn('_search()', 'Orders Screen','all');
