...
class ClosedTransactionScreen extends StatefulWidget{

  @override
  State<ClosedTransactionScreen> createState() => _ClosedTransactionScreenState();
}

class _ClosedTransactionScreenState extends State<ClosedTransactionScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      setNotification.value = NotificationMessage(message:  MessagesProvider.get("View &1 transactions ",['Closed']),color:theme.footerBG4);
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    ).withLoader(theme,isLoading: _isLoading, message: MessagesProvider.get("Loading Closed Transactions"));
  }
  ...
  _search() async{
    ...
    if(_fromDateController.text != ""){
      ...
      fromDate = DateFormat('yyyy-MM-ddThh:mm:ss').format(from);
    ...
    if(_toDateController.text != ""){
      ...
      toDate = DateFormat('yyyy-MM-ddThh:mm:ss').format(from);
    }
    ...
    try{
      ...
      var response = await orderSearchBL.getTransaction(
        ...
        status: (_status?.statusDescription == null || _status?.statusDescription == "All") ? "CLOSED,CANCELLED,REVERSED" : _status?.statusDescription,
      ...
      if(response.exception == null) {
        Log.printMethodReturn('_search() - done!', 'Orders Screen','closed');
