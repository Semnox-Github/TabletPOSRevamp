...
class _GameActivityState extends State<GameActivity> {
  ...
  List<SummariesData>? _summariesData;
  CustomerData? data;
  ...
  Future _getSummaries(BuildContext context) async {
    setLoadingStatus(true);
    final execContextBL = await ExecutionContextBuilder.build();
    final execContext = execContextBL.getExecutionContext();

    final customerDataBl = await CustomerDataBuilder.build(execContext!);
    summaries = await customerDataBl.getGameSummary(accountId: cardData?.accountId ?? -1, currentPage: _pageNumber, pageSize: _pageSize, toDate: _getFormattedDateForAPI(_toDateCtr.text), fromDate: _getFormattedDateForAPI(_fromDateCtr.text)
    );
    setState(() {
      setLoadingStatus(false);
      _summariesData = summaries?.data;
      _notificationBar?.showMessage(
          MessagesProvider.get('Search completed successfully.'),
  ...
  Future _getSummariesCounts(BuildContext context) async {
    setLoadingStatus(true);
    final execContextBL = await ExecutionContextBuilder.build();
    final execContext = execContextBL.getExecutionContext();

    final customerDataBl = await CustomerDataBuilder.build(execContext!);
    counts = await customerDataBl.getGameSummaryCounts(accountId: cardData?.accountId ?? -1, toDate: _getFormattedDateForAPI(_toDateCtr.text), fromDate: _getFormattedDateForAPI(_fromDateCtr.text),
    );
    setState(() {
      setLoadingStatus(false);
      if((counts?.data ?? 0) > 0){
  ...
  void initState() {
    ...
    _pageSizeController.text = MessagesProvider.get('50');
            ...
            Row(mainAxisAlignment: MainAxisAlignment.end, children: [
              ...
              InkWell(
                ...
                child: Container(
                  ...
                  child: Text(
                    (_summariesData?.length ?? 0) == 0 ?(_pageNumber).toString() :(_pageNumber + 1).toString(),
              ...
              Text(
                ((_summariesData?.length ?? 0) > 0 ? '$_totalPages':0).toString()                         ,
                style: theme.heading5?.copyWith(
                    fontSize: SizeConfig.getFontSize(18),
                    fontStyle: FontStyle.normal,
                    fontWeight: FontWeight.w500
              ...
              Container(
                ...
                child: ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    ...
                    backgroundColor: (_summariesData?.length ?? 0) > 0 ? theme.button2BG1 :theme.button1BG1,
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(4.0)),
                  ),
                  child: Text(MessagesProvider.get('GO'), style:(_summariesData?.length ?? 0) > 0 ? AppStyle.customerTextBoldWhite?.copyWith(fontSize: SizeConfig.getFontSize(18)): AppStyle.customerDisableText?.copyWith(fontSize: SizeConfig.getFontSize(18))
                  ),),
              )
                    ...
                    Expanded(
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            CustomerDatePicker(
                                title : MessagesProvider.get('From'),hint:  DateFormat(_dateFormat ?? 'dd-MM-yyyy').format(DateTime.now()),
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: ElevatedButton(
                              onPressed: ()  {
                                if((_summariesData?.length ?? 0) > 0){
                                  _showErrorDialogView(context, MessagesProvider.get('WARNING'),
                                      MessagesProvider.get('The search criteria is modified. Do you want to print for the updated search criteria?'),2,theme);
                                  setState(() {});
                                }
                              } ,
                              style: ElevatedButton.styleFrom(
                              ...
                              child: Text(MessagesProvider.get('PRINT'), style:(_summariesData?.length ?? 0) > 0 ? AppStyle.customerTextBoldWhite?.copyWith(fontSize: SizeConfig.getFontSize(18)): AppStyle.customerDisableText?.copyWith(fontSize: SizeConfig.getFontSize(18))
                                ,),),
                          ),
                        ],
                      ),
                                  ...
                                  children: [
                                    Row(
                                      ...
                                      children: [
                                        ...
                                        Padding(
                                          padding:
                                          EdgeInsets.only(left: SizeConfig.getSize(165)),
                                          child: Text(
                                            MessagesProvider.get('Machine'),
                                            style: theme.heading5?.copyWith( fontSize: SizeConfig.getFontSize(18),fontWeight: FontWeight.w700),
                                            overflow: TextOverflow.ellipsis,
                                          ),
                                        ),
                                        ...
                                        Padding(
                                          padding:
                                          EdgeInsets.only(left: SizeConfig.getSize(65)),
                                          child: Text(
                                        ...
                                        Padding(
                                          ...
                                          EdgeInsets.only(left: SizeConfig.getSize(20)),
                                          child: Text(
                                            MessagesProvider.get('e-Tickets'),
                                            style: theme.heading5?.copyWith( fontSize: SizeConfig.getFontSize(18),fontWeight: FontWeight.w700),
                                            overflow: TextOverflow.ellipsis,
                                          ),
                                        ),
                                        Padding(
                                          ...
                                          EdgeInsets.only(left: SizeConfig.getSize(20)),
                                          child: Text(
                                            MessagesProvider.get('Manual Tickets'),
                                            style: theme.heading5?.copyWith( fontSize: SizeConfig.getFontSize(18),fontWeight: FontWeight.w700),
                                            overflow: TextOverflow.ellipsis,
                                          ),
                                        ),
                                        Padding(
                                          ...
                                          EdgeInsets.only(left: SizeConfig.getSize(20)),
                                          child: Text(
                                            MessagesProvider.get('T.Eater Tickets'),
                                            style: theme.heading5?.copyWith( fontSize: SizeConfig.getFontSize(18),fontWeight: FontWeight.w700),
                                            overflow: TextOverflow.ellipsis,
                                          ),
                                        ),
                                    ...
                                    ListView.builder(
                                        ...
                                        itemCount:  _summariesData?.length ?? 0,
                                        itemBuilder: (BuildContext context, int index) {
                                          return Padding(
                                            padding: const EdgeInsets.all(3.0),
                                            child:  SummariesSearchListItem(
  ...
  _showErrorDialogView(BuildContext context, String title, String message, int type, SemnoxTheme theme) {
    showDialog(
        context: context, builder: (_) =>  _showDialogWidget(title,message,type,theme));
  }
  ...
  _showDialogWidget(String title,String message, int type, themes){
    SemnoxTheme theme = Theme.of(context).extension<SemnoxTheme>()!;
    return AlertDialog(
      title:  Padding(
      ...
      content: _widget(context,title,message,theme),
              ...
              if(type == 1) {
                ...
                if (daysBetween(DateTime.parse(
                    ...
                    DateTime.parse(_getFormattedDate(
                        _toDateCtr.text))) >= 365) {
                  _showErrorDialogView(context, MessagesProvider.get('WARNING'),
                      MessagesProvider.get('Date range provided is greater than a year. Do you want to proceed with search?'),3,theme);
                } else {
                  _notificationBar?.showMessage(
  ...
  Widget _widget(BuildContext context, String title, String message, themes){
    SemnoxTheme theme = Theme.of(context).extension<SemnoxTheme>()!;
