...
class CardEntryPopup extends StatefulWidget {
  final Function(String) onCardNumEntered;

  const CardEntryPopup({
    super.key,
    required this.onCardNumEntered
  });

  @override
  State<StatefulWidget> createState() => _CardEntryPopupState();
}
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                containerButton(
                                    ...
                                    onTapped: () {
                                      FocusManager.instance.primaryFocus?.unfocus();
                                      final cardNo = _cardNumberCtr.text.trim();
                                      if (cardNo.isEmpty) {
                                        _notificationBar.showMessage(MessagesProvider.get('Please enter required fields'), AppColors.notificationBGRedColor);
                                        return;
                                      }
                                      if (cardNo.length != 8) {
                                        _notificationBar.showMessage(MessagesProvider.get('Invalid Card Number length (&1). Should be &2', [_cardNumberCtr.text.length, 8]), AppColors.notificationBGRedColor);
                                        return;
                                      }
                                      _notificationBar.showMessage(
                                          '',
                                          AppColors.primaryColor);
                                      widget.onCardNumEntered(cardNo);
                                      Navigator.pop(context);
